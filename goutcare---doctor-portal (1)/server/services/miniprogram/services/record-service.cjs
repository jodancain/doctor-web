...
async function addRecord(context) {
  ...
  try {
  const res = await db.collection(collectionName).add(record);
...
async function getHomeSummary(context) {
  ...
  const [uaRes, attackRes, waterRes, exerciseRes, medicationRes] = await Promise.all([
    ...
    db.collection('attackRecords').where({ _openid: openid, createdAt: _.gte(sevenDaysAgo) }).count(),
    db.collection('waterRecords').where({ _openid: openid, createdAt: _.gte(sevenDaysAgo) }).orderBy('createdAt', 'desc').limit(100).get(),
    db.collection('exerciseRecords').where({ _openid: openid, createdAt: _.gte(sevenDaysAgo) }).orderBy('createdAt', 'desc').limit(100).get(),
    db.collection('medicationReminders').where({ _openid: openid, status: 'active' }).count()
  ]);

  const latestUa = uaRes.data[0];
  const waterTotal = waterRes.data.reduce((acc, cur) => acc + (cur.amount || 0), 0);
